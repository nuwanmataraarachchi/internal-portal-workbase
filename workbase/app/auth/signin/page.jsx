...
export default function SignInPage() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      setError(axios.isAxiosError(requestError) ? requestError.response?.data?.message || "We could not sign you in. Please try again." : "Something went wrong. Check your connection and try again.");
    } finally {
    ...
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-slate-50 px-5 py-10 text-slate-900">
      ...
      <section className="relative w-full max-w-[440px] rounded-3xl border border-white/80 bg-white/90 p-7 shadow-2xl shadow-slate-900/10 backdrop-blur sm:p-10">
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          <label className="block"><span className="mb-2 block text-sm font-medium text-slate-700">Username</span><input className="h-12 w-full rounded-xl border border-slate-200 bg-white px-4 text-sm outline-none transition placeholder:text-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-100" type="text" autoComplete="username" value={username} onChange={(event) => setUsername(event.target.value)} placeholder="Admin" required /></label>
        ...
        <div className="mt-7 rounded-xl border border-slate-100 bg-slate-50 px-4 py-3 text-xs leading-5 text-slate-500"><span className="font-semibold text-slate-700">Admin access</span> &middot; Admin / Admin@123</div>
      </section>
    </main>
  );
}
